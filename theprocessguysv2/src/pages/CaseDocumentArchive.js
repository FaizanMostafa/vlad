...
import { getUserCases } from "../redux/actions/case";
...
function CaseDocumentArchive() {
  ...
  useEffect(() => {
    if (!cases.length && !hasFetchedCases) {
      dispatch(
        getUserCases({ uid: user.uid }, () => {
          setHasFetchedCases(true);
        })
      );
    }
  }, [cases]);
  ...
  return (
    <MDBCol style={{ minHeight: "100vh" }} className="">
      ...
      <h1 className="text-center">
        <b>Case Document Archive</b>
      </h1>
      <br></br>
      <br></br>
      <h4 className="text-center">Search Cases</h4>
      <br></br>
      <input
      ...
      {!isFetching && cases.length ? (
        cases
          .filter((caseDoc) => caseDoc?.searchString?.includes(searchTerm))
          .map((caseData, index) => (
            <Accordian index={index} caseData={caseData} />
          ))
      ) : (
        <center>
          <b>No case data available</b>
        </center>
      )}
