...
const Notifications = () => {
  ...
  const lastVisible = useSelector(
    (state) => state.admin.lastNotificationVisible
  );
  const isFetchingNotifications = useSelector(
    (state) => state.admin.isFetchingNotifications
  );
  ...
  const handleActivePageNoChanged = (newActivePageNo) => {
    if (
      metadata.Notifications > activePageNo * noOfRowsPerPage &&
      notifications.length < metadata.Notifications
    ) {
  ...
  const handleNoOfRowsPerPageChanged = (newNoOfRowsPerPage) => {
    if (
      ...
      console.log(
        "Fetching next chunk of Notifications for changed no of rows per page..."
      );
      const data = {
  ...
  const handleOnClickViewCaseDetails = () => {
    if (
      !userCase ||
      userCase.docId !== notification.content.caseId ||
      ((!userCase || userCase.docId !== notification.content.caseId) &&
        (!caseDetails || caseDetails.caseId !== notification.content.caseId))
    ) {
      dispatch(
        fetchCase(
          { caseId: notification.content.caseId },
          (userCase) => dispatch(fetchCaseDetails(userCase)),
          () => setEditCaseModalShow(false)
        )
      );
    } else if (
      !caseDetails ||
      caseDetails.caseId !== notification.content.caseId
    ) {
            ...
            notifications
              ...
              .map((notification, index) => (
                ...
                >
                  ...
                  <td>
                    {new Date(notification.generatedAt.toDate()).toDateString()}
                  </td>
