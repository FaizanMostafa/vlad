...
import { updateCase, createCase } from "../../redux/actions/admin";
...
export const FileSubmission = ({
  ...
  const [showCancelCaseConfirmationModal, setShowCancelCaseConfirmationModal] =
    useState(false);
  ...
  useEffect(() => {
    if (props.fileData && props.numberOfCaseFilesBeingServed) {
      ...
      for (
        let index = Object.keys(caseFileData).length;
        index < parseInt(props.numberOfCaseFilesBeingServed);
        index++
      ) {
        caseFileData[index] = {
          file: null,
          ...
          fileContents: {
            ...
            complaint: false,
            contract: false,
            alternativeDisputeResolution: false,
            exhibit: false,
            dissolutionOfMarriage: false,
            temporaryRestrainingOrder: false,
      ...
      const numberOfCaseFilesBeingServed = JSON.parse(
        localStorage.getItem("Questionaire4")
      ).numberOfCaseFilesBeingServed;
      let caseFileData = {};
      for (
        let index = 0;
        index < parseInt(numberOfCaseFilesBeingServed);
        index++
      ) {
        caseFileData[index] = {
          file: null,
  ...
  const handleOnClickSubmit = () => {
    if (Object.values(fileData).filter((o) => !o.caseType.length).length) {
      showToast(
        "Please enter the case type in every relevant input field!",
        "warning"
      );
    } else if (
      ...
      showToast(
        "Please upload the files in every relevant input field!",
        "warning"
      );
    } else if (!isUpdating && !isCreating) {
      let data = {};
      const QuestionaireForm1 = JSON.parse(
        localStorage.getItem("Questionaire1")
      );
      const QuestionaireForm2 = JSON.parse(
        localStorage.getItem("Questionaire2")
      );
      const QuestionaireForm3 = JSON.parse(
        localStorage.getItem("Questionaire3")
      );
      const QuestionaireForm4 = JSON.parse(
        localStorage.getItem("Questionaire4")
      );
      ...
      const QuestionaireForm8 = JSON.parse(
        localStorage.getItem("Questionaire8")
      );
      if (QuestionaireForm1 && Object.values(QuestionaireForm1).length) {
        ...
        if (QuestionaireForm1?.ownerOfService)
          data["CaseInformation-1"].ownerOfService =
            QuestionaireForm1.ownerOfService;
        if (QuestionaireForm1.hasOwnProperty("status"))
          data["CaseInformation-1"].status = QuestionaireForm1.status;
        if (QuestionaireForm1?.amount)
          data["CaseInformation-1"].amount = QuestionaireForm1.amount;
        if (QuestionaireForm1.hasOwnProperty("caseTitle"))
          data["CaseInformation-1"].caseTitle = QuestionaireForm1.caseTitle;
        if (QuestionaireForm1.hasOwnProperty("caseNumber"))
          data["CaseInformation-1"].caseNumber = QuestionaireForm1.caseNumber;
        if (QuestionaireForm1.hasOwnProperty("courtDate"))
          data["CaseInformation-1"].courtDate = QuestionaireForm1.courtDate;
        if (QuestionaireForm1.hasOwnProperty("courtType"))
          data["CaseInformation-1"].courtType = QuestionaireForm1.courtType;
        if (QuestionaireForm1.hasOwnProperty("courtState"))
          data["CaseInformation-1"].courtState = QuestionaireForm1.courtState;
        if (QuestionaireForm1.hasOwnProperty("countyOf"))
          data["CaseInformation-1"].countyOf = QuestionaireForm1.countyOf;
        if (QuestionaireForm1.hasOwnProperty("courthouseAddress"))
          data["CaseInformation-1"].courthouseAddress =
            QuestionaireForm1.courthouseAddress;
        if (QuestionaireForm1.hasOwnProperty("courthouseMailingAddress"))
          data["CaseInformation-1"].courthouseMailingAddress =
            QuestionaireForm1.courthouseMailingAddress;
        if (QuestionaireForm1.hasOwnProperty("branchName"))
          data["CaseInformation-1"].branchName = QuestionaireForm1.branchName;
      }
      if (QuestionaireForm2 && Object.values(QuestionaireForm2).length) {
        ...
        if (QuestionaireForm2.hasOwnProperty("plaintiffsDetail"))
          data["PlaintiffInformation-2"].plaintiffsDetail =
            QuestionaireForm2.plaintiffsDetail;
        if (QuestionaireForm2.hasOwnProperty("numberOfAttorneyPlaintiff"))
          data["PlaintiffInformation-2"].numberOfAttorneyPlaintiff =
            QuestionaireForm2.numberOfAttorneyPlaintiff;
        if (QuestionaireForm2.hasOwnProperty("isOrRepresentingPlaintiff"))
          data["PlaintiffInformation-2"].isOrRepresentingPlaintiff =
            QuestionaireForm2.isOrRepresentingPlaintiff;
        if (QuestionaireForm2.hasOwnProperty("shouldPGFillPlaintiffInfo"))
          data["PlaintiffInformation-2"].shouldPGFillPlaintiffInfo =
            QuestionaireForm2.shouldPGFillPlaintiffInfo;
        if (
          QuestionaireForm2.hasOwnProperty(
            "numberOfAttorneysRepresentingPlaintiff"
          )
        )
          data[
            "PlaintiffInformation-2"
          ].numberOfAttorneysRepresentingPlaintiff =
            QuestionaireForm2.numberOfAttorneysRepresentingPlaintiff;
        if (QuestionaireForm2.hasOwnProperty("plaintiffAttorneysDetail"))
          data["PlaintiffInformation-2"].plaintiffAttorneysDetail =
            QuestionaireForm2.plaintiffAttorneysDetail;
      }
      if (QuestionaireForm3 && Object.values(QuestionaireForm3).length) {
        ...
        if (QuestionaireForm3.hasOwnProperty("defendantsDetail"))
          data["DefendantInformation-3"].defendantsDetail =
            QuestionaireForm3.defendantsDetail;
        if (QuestionaireForm3.hasOwnProperty("numberOfAttorneyDefendant"))
          data["DefendantInformation-3"].numberOfAttorneyDefendant =
            QuestionaireForm3.numberOfAttorneyDefendant;
        if (QuestionaireForm3.hasOwnProperty("isOrRepresentingDefendant"))
          data["DefendantInformation-3"].isOrRepresentingDefendant =
            QuestionaireForm3.isOrRepresentingDefendant;
        if (QuestionaireForm3.hasOwnProperty("shouldPGFillDefendantInfo"))
          data["DefendantInformation-3"].shouldPGFillDefendantInfo =
            QuestionaireForm3.shouldPGFillDefendantInfo;
        if (
          QuestionaireForm3.hasOwnProperty(
            "numberOfAttorneysRepresentingDefendant"
          )
        )
          data[
            "DefendantInformation-3"
          ].numberOfAttorneysRepresentingDefendant =
            QuestionaireForm3.numberOfAttorneysRepresentingDefendant;
        if (QuestionaireForm3.hasOwnProperty("defendantAttorneysDetail"))
          data["DefendantInformation-3"].defendantAttorneysDetail =
            QuestionaireForm3.defendantAttorneysDetail;
      }
      if (QuestionaireForm4 && Object.values(QuestionaireForm4).length) {
        ...
        if (QuestionaireForm4.hasOwnProperty("numberOfCaseFilesBeingServed"))
          data["ServeeDocumentedData-4"].numberOfCaseFilesBeingServed =
            QuestionaireForm4.numberOfCaseFilesBeingServed;
        if (QuestionaireForm4.hasOwnProperty("howManyIndividualsServed"))
          data["ServeeDocumentedData-4"].howManyIndividualsServed =
            QuestionaireForm4.howManyIndividualsServed;
        if (QuestionaireForm4.hasOwnProperty("serveesDetail"))
          data["ServeeDocumentedData-4"].serveesDetail =
            QuestionaireForm4.serveesDetail;
      }
      if (QuestionaireForm5 && Object.values(QuestionaireForm5).length) {
        ...
        if (QuestionaireForm5.hasOwnProperty("serveIndividualAtEmployment"))
          data["ClearanceOfAction-5"].serveIndividualAtEmployment =
            QuestionaireForm5.serveIndividualAtEmployment;
        if (
          QuestionaireForm5.hasOwnProperty(
            "requireServerNotifyPersonOfInterest"
          )
        )
          data["ClearanceOfAction-5"].requireServerNotifyPersonOfInterest =
            QuestionaireForm5.requireServerNotifyPersonOfInterest;
        if (QuestionaireForm5.hasOwnProperty("serverContactServeeByPhone"))
          data["ClearanceOfAction-5"].serverContactServeeByPhone =
        ...
        if (
          ...
          data["ClearanceOfAction-5"].paralegalAttorneyClientContactServee =
            QuestionaireForm5.paralegalAttorneyClientContactServee;
      }
      if (QuestionaireForm6 && Object.values(QuestionaireForm6).length) {
        if (props.fileData && props.numberOfCaseFilesBeingServed) {
          if (QuestionaireForm6.hasOwnProperty("serveesPhysicalDescription"))
            data["ServeePhysicalDescription-6"] = {
              docId: QuestionaireForm6.docId,
      ...
      if (QuestionaireForm7 && Object.values(QuestionaireForm7).length) {
        if (props.fileData && props.numberOfCaseFilesBeingServed) {
          if (QuestionaireForm7.hasOwnProperty("vehiclesInformation"))
            data["VehicleInformation-7"] = {
              docId: QuestionaireForm7.docId,
      ...
      if (QuestionaireForm8 && Object.values(QuestionaireForm8).length) {
        ...
        if (QuestionaireForm8.hasOwnProperty("specifyDatesForStakeOutService"))
          data["OfferedServices-8"].specifyDatesForStakeOutService =
            QuestionaireForm8.specifyDatesForStakeOutService;
        if (QuestionaireForm8.hasOwnProperty("requireSkipTracingService"))
          data["OfferedServices-8"].requireSkipTracingService =
            QuestionaireForm8.requireSkipTracingService;
        if (QuestionaireForm8.hasOwnProperty("requireBodyCamFootage"))
          data["OfferedServices-8"].requireBodyCamFootage =
            QuestionaireForm8.requireBodyCamFootage;
        if (QuestionaireForm8.hasOwnProperty("obtainNewDeliveryLocation"))
          data["OfferedServices-8"].obtainNewDeliveryLocation =
            QuestionaireForm8.obtainNewDeliveryLocation;
        if (QuestionaireForm8.hasOwnProperty("poBoxAllowedToServe"))
          data["OfferedServices-8"].poBoxAllowedToServe =
            QuestionaireForm8.poBoxAllowedToServe;
        if (QuestionaireForm8.hasOwnProperty("requireServiceByMail"))
          data["OfferedServices-8"].requireServiceByMail =
            QuestionaireForm8.requireServiceByMail;
        if (QuestionaireForm8.hasOwnProperty("requireByEmail"))
          data["OfferedServices-8"].requireByEmail =
            QuestionaireForm8.requireByEmail;
        if (QuestionaireForm8.hasOwnProperty("specificCourtInstruction"))
          data["OfferedServices-8"].specificCourtInstruction =
            QuestionaireForm8.specificCourtInstruction;
      ...
      if (
        Object.values(fileData).filter(
          (fileObj) =>
            fileObj.hasOwnProperty("file") ||
            !objectsEqual(fileObj, props.fileData[0])
        ).length
      ) {
        if (props.docId) {
        ...
        Object.values(fileData).forEach((fileObj) => {
          if (fileObj.hasOwnProperty("file")) {
            if (!data["FileSubmission-9"].hasOwnProperty("oldDocumentPath"))
              data["FileSubmission-9"].oldDocumentPath = props.documentPath;
            if (data["FileSubmission-9"].hasOwnProperty("documents")) {
              data["FileSubmission-9"].documents.push({
                ...fileObj,
                fileType: fileSubmissionType,
              });
            } else {
              data["FileSubmission-9"].documents = [
                { ...fileObj, fileType: fileSubmissionType },
              ];
            }
          } else {
            data["FileSubmission-9"].documents = Object.values(fileData).map(
              (fd) => ({ ...fd, fileType: fileSubmissionType })
            );
          }
        });
      }
      if (Object.keys(data).length) {
        ...
        if (props.fileData && props.numberOfCaseFilesBeingServed) {
          data.caseId = caseId;
          if (
            data.hasOwnProperty("CaseInformation-1") &&
  ...
  return !isFormDisabled && fileSubmissionType === "" ? (
    <>
      <center>
        <h2>
      ...
      <div>
        ...
        <MDBCol md="12">
          <center>
            <Button
              onClick={() => setFileSubmissionType("single")}
              className="w-75"
            >
            ...
            <br />
            <Button
              onClick={() => setFileSubmissionType("multiple")}
              className="w-75"
            >
    ...
    <Fragment>
      ...
      <center>
        <h2>
          <b>Case Files Submission Upload</b>
        </h2>
      </center>
      <div>
        <br />
        <MDBCol md="12">
          {Object.entries(fileData).map(([key, value]) => (
            <div>
              <Form.Group id="mS-file-upload">
                ...
                <Form.Control
                  ...
                  onChange={(e) => {
                    setFileData({
                      ...fileData,
                      [key]: { ...fileData[key], caseType: e.target.value },
                    });
                  }}
                />
              </Form.Group>
              <Form.Group id="mS-file-upload">
                ...
                <div
                  style={{ display: "flex", flexWrap: "wrap", paddingLeft: 30 }}
                >
                  <div style={{ width: 300 }}>
                    <Form.Check
                      type="checkbox"
                        ...
                        setFileData({
                          ...
                          [key]: {
                            ...
                            fileContents: {
                              ...
                              coverSheet:
                                !fileData[key].fileContents.coverSheet,
                            },
                          },
                        });
                      }}
                        ...
                        setFileData({
                          ...
                          [key]: {
                            ...
                            fileContents: {
                              ...fileData[key].fileContents,
                              civilCoverSheet:
                                !fileData[key].fileContents.civilCoverSheet,
                            },
                          },
                        });
                      }}
                      ...
                      onChange={(e) => {
                        setFileData({
                          ...
                          [key]: {
                            ...
                            fileContents: {
                              ...fileData[key].fileContents,
                              summons: !fileData[key].fileContents.summons,
                            },
                          },
                        });
                      }}
                      ...
                      onChange={(e) => {
                        setFileData({
                          ...
                          [key]: {
                            ...
                            fileContents: {
                              ...fileData[key].fileContents,
                              complaint: !fileData[key].fileContents.complaint,
                            },
                          },
                        });
                      }}
                      ...
                      onChange={(e) => {
                        setFileData({
                          ...
                          [key]: {
                            ...
                            fileContents: {
                              ...fileData[key].fileContents,
                              contract: !fileData[key].fileContents.contract,
                            },
                          },
                        });
                      }}
                      ...
                      onChange={(e) => {
                        setFileData({
                          ...
                          [key]: {
                            ...
                            fileContents: {
                              ...fileData[key].fileContents,
                              exhibit: !fileData[key].fileContents.exhibit,
                            },
                          },
                        });
                      }}
                        ...
                        setFileData({
                          ...
                          [key]: {
                            ...
                            fileContents: {
                              ...fileData[key].fileContents,
                              restrainingOrder:
                                !fileData[key].fileContents.restrainingOrder,
                            },
                          },
                        });
                      }}
                      ...
                      onChange={(e) => {
                        setFileData({
                          ...
                          [key]: {
                            ...
                            fileContents: {
                              ...fileData[key].fileContents,
                              petition: !fileData[key].fileContents.petition,
                            },
                          },
                        });
                      }}
                        ...
                        setFileData({
                          ...
                          [key]: {
                            ...
                            fileContents: {
                              ...fileData[key].fileContents,
                              statementOfLocation:
                                !fileData[key].fileContents.statementOfLocation,
                            },
                          },
                        });
                      }}
                        ...
                        setFileData({
                          ...
                          [key]: {
                            ...
                            fileContents: {
                              ...fileData[key].fileContents,
                              declarationOfVenue:
                                !fileData[key].fileContents.declarationOfVenue,
                            },
                          },
                        });
                      }}
              ...
              <div>
                ...
                value.fileType.toLowerCase() === "single" ? (
                  <>
                    ...
                    >
                      {isFileVisible === value.documentName ? "Hide" : "View"}{" "}
                      File
                    </span>
                    <span
                      onClick={() =>
                        downloadResource(
                          props.documentURI,
                          props.documentURI.slice(
                            props.documentURI.lastIndexOf("%2F") + 3,
                            props.documentURI.lastIndexOf("?")
                          )
                        )
                      }
                      style={{
                  ...
                  <>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        setFileData({
                          ...fileData,
                          [key]: { ...fileData[key], file: e.target.files[0] },
                        });
                      }}
                      accept={
          ...
          {!isFormDisabled && (
            <b>
              Any file uploaded must contain all case documents organized and
              ready to be served. You may upload multiple case files, if more
              than one case is being served.
            </b>
          )}
        </MDBCol>
        <br />
          ...
          <div style={{ display: "flex", justifyContent: "flex-end" }}>
            ...
            >
              {isUpdating || isCreating ? (
                ...
                <span className="text-white">
                  {props.fileData && props.numberOfCaseFilesBeingServed
                    ? "Update"
                    : "Create"}{" "}
                  Case
                </span>
      ...
      <CaseCancelConfirmation
        ...
        handleOnPressProceed={onSubmitFunction}
      />
    </Fragment>
  );
};
